...
class Controller:
  ...
  def get_action(self, h):
    ...
    #print("lenh", len(h))
    #print("lensw", len(self.weight))
    #print("lenb", len(self.bias))
    #print("ssw", self.weight.shape)
    #print("ssb", self.bias.shape)
...
def simulate(controller, env, train_mode=False, render_mode=True, num_episode=5, seed=-1, max_len=-1):
  ...
  for episode in range(num_episode):
    #if train_mode and episode == num_episode-1: 
    #  print('episode: {}/{}'.format(episode, num_episode))
    obs = env.reset()
    if len(obs) == 2:
      obs = obs[0]
    #obs = env._process_frame(obs)
    total_reward = 0.0
